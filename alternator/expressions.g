...
@lexer::traits {
    ...
    public:
        typedef customTokenStream<ImplTraits> TokenStreamType;
        typedef customTokenStream<ImplTraits> TokenIntStreamType;
        typedef customInputStream<ImplTraits> InputStreamType;
    };
    typedef antlr3::Traits<expressionsLexer, expressionsParser, userTraits> expressionsLexerTraits;
}
@parser::traits {
...
// Make all tokens explicit to generate enum values, needed for custom lexer
OPEN_BRACKET: '(';
CLOSE_BRACKET: ')';
OPEN_SQUARE_BRACKET: '[';
...
path returns [parsed::path p]:
    ...
    (   DOT name=path_component   { $p.add_dot($name.text); }
      | OPEN_SQUARE_BRACKET INTEGER CLOSE_SQUARE_BRACKET { $p.add_index(std::stoi($INTEGER.text)); }
    )*;
...
value returns [parsed::value v]:
    ...
    | NAME         { $v.set_func_name($NAME.text); }
     OPEN_BRACKET x=value   { $v.add_func_parameter($x.v); }
     (COMMA x=value  { $v.add_func_parameter($x.v); })*
     CLOSE_BRACKET
    ;

update_expression_set_rhs returns [parsed::set_rhs rhs]:
    v=value  { $rhs.set_value(std::move($v.v)); }
    (   PLUS  v=value  { $rhs.set_plus(std::move($v.v)); }
      | MINUS v=value  { $rhs.set_minus(std::move($v.v)); }
    )?
    ;

update_expression_set_action returns [parsed::update_expression::action a]:
    path EQ rhs=update_expression_set_rhs { $a.assign_set($path.p, $rhs.rhs); };
...
update_expression_clause returns [parsed::update_expression e]:
      ...
      (COMMA s=update_expression_set_action { $e.add(s); })*
    | REMOVE r=update_expression_remove_action { $e.add(r); }
      (COMMA r=update_expression_remove_action { $e.add(r); })*
    | ADD a=update_expression_add_action { $e.add(a); }
      (COMMA a=update_expression_add_action { $e.add(a); })*
    | DELETE d=update_expression_delete_action { $e.add(d); }
      (COMMA d=update_expression_delete_action { $e.add(d); })*
    ;
...
projection_expression returns [std::vector<parsed::path> v]:
    ...
    (COMMA p=path { $v.push_back(std::move($p.p)); } )* EOF;
...
primitive_condition returns [parsed::primitive_condition c]:
      ...
      (  (  EQ       { $c.set_operator(parsed::primitive_condition::type::EQ); }
          | LESS MORE   { $c.set_operator(parsed::primitive_condition::type::NE); }
          | LESS       { $c.set_operator(parsed::primitive_condition::type::LT); }
          | LESS EQ   { $c.set_operator(parsed::primitive_condition::type::LE); }
          | MORE       { $c.set_operator(parsed::primitive_condition::type::GT); }
          | MORE EQ   { $c.set_operator(parsed::primitive_condition::type::GE); }
         )
         v=value      { $c.add_value(std::move($v.v)); }
       | BETWEEN      { $c.set_operator(parsed::primitive_condition::type::BETWEEN); }
         ...
         AND
         v=value      { $c.add_value(std::move($v.v)); }
       | IN OPEN_BRACKET       { $c.set_operator(parsed::primitive_condition::type::IN); }
         v=value      { $c.add_value(std::move($v.v)); }
         (COMMA v=value { $c.add_value(std::move($v.v)); })*
         CLOSE_BRACKET
      )?
    ;
...
boolean_expression_2 returns [parsed::condition_expression e]:
	...
	| OPEN_BRACKET b=boolean_expression CLOSE_BRACKET { $e = std::move($b.e); }
    ;
