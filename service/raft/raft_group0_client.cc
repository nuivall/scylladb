...
future<> raft_group0_client::broadcast_group0_read_barrier(seastar::abort_source& as) {
    ...
    co_await coroutine::parallel_for_each(live_members, [&] (locator::host_id host) -> future<> {
        ...
        try {
            ...
            co_await rpc.send_group0_read_barrier(dst);
        } catch (...) {
...
future<> group0_batch::commit(::service::raft_group0_client& group0_client, seastar::abort_source& as,
        std::optional<::service::raft_timeout> timeout, group0_batch_barrier barrier) && {
    ...
    if (barrier == group0_batch_barrier::global) {
        co_await group0_client.broadcast_group0_read_barrier(as);
    }
}
