...
future<locator::mutable_token_metadata_ptr> storage_service::prepare_update_tablet_metadata(const locator::tablet_metadata_change_hint& hint) {
    SCYLLA_ASSERT(this_shard_id() == 0);
    auto tmptr = co_await get_mutable_token_metadata_ptr();
    if (hint) {
...
future<> storage_service::commit_update_tablet_metadata(locator::mutable_token_metadata_ptr tmptr, const bool wake_up_load_balancer) {
    co_await replicate_to_all_cores(std::move(tmptr));
    if (wake_up_load_balancer) {
        _topology_state_machine.event.broadcast();
    }
}

future<> storage_service::update_tablet_metadata(const locator::tablet_metadata_change_hint& hint, const bool wake_up_load_balancer) {
    co_await commit_update_tablet_metadata(
            co_await prepare_update_tablet_metadata(hint), wake_up_load_balancer);
}
