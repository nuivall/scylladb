...
future<> schema_applier::finalize_tables_and_views() {
    ...
    co_await max_concurrent_for_each(diff.local().tables.created, max_concurrent, [this] (const schema_ptr& gs) -> future<> {
        co_await store_column_mapping(_proxy, gs, false);
    });
    co_await max_concurrent_for_each(diff.local().tables.altered, max_concurrent, [this] (const schema_diff_per_shard::altered_schema& altered) -> future<> {
    ...
    co_await max_concurrent_for_each(diff.local().tables.dropped, max_concurrent, [this] (const schema_ptr& s) -> future<> {
