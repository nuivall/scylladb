...
in_progress_types_storage_per_shard::in_progress_types_storage_per_shard(const replica::database& db, const affected_keyspaces& affected_keyspaces, const affected_user_types& affected_types) : _stored_user_types(db.user_types()) {
    ...
    for (auto &ks : affected_keyspaces.names.dropped) {
...
future<> in_progress_types_storage::init(distributed<replica::database>& sharded_db, const affected_keyspaces& affected_keyspaces, const affected_user_types& affected_types) {
    co_await sharded_db.invoke_on_all([&] (replica::database& db) {
        shards[this_shard_id()] = seastar::make_shared<in_progress_types_storage_per_shard>(db, affected_keyspaces, affected_types);
    });
}

shared_ptr<in_progress_types_storage_per_shard> in_progress_types_storage::local() {
...
static future<functions_change_batch_all_shards> merge_functions(distributed<service::storage_proxy>& proxy, schema_result before, schema_result after, in_progress_types_storage& types_storage) {
    ...
    co_await proxy.local().get_db().invoke_on_all(coroutine::lambda([&] (replica::database& db) -> future<> {
        ...
        batch.user_types = types_storage.local();
        for (const auto& val : diff.created) {
            batch.add_function(co_await create_func(db, *val, *batch.user_types));
        }
        for (const auto& val : diff.dropped) {
            cql3::functions::function_name name{
                val->get_nonnull<sstring>("keyspace_name"), val->get_nonnull<sstring>("function_name")};
            auto arg_types = read_arg_types(*val, name.keyspace, *batch.user_types);
        ...
        for (const auto& val : diff.altered) {
            drop_cached_func(db, *val);
            batch.replace_function(co_await create_func(db, *val, *batch.user_types));
        }
    }));
    co_return batches;
}
...
static future<functions_change_batch_all_shards> merge_aggregates(distributed<service::storage_proxy>& proxy, const schema_result& before, const schema_result& after,
    ...
    co_await proxy.local().get_db().invoke_on_all([&] (replica::database& db)-> future<> {
        ...
        for (const auto& val : diff.dropped) {
            ...
            auto arg_types = read_arg_types(*val.first, name.keyspace, *batch.user_types);
            batch.remove_function(name, arg_types);
        }
        co_return;
    });
...
void schema_applier::commit_on_shard(replica::database& db) {
    ...
    for (auto& ks_name : _affected_keyspaces.names.dropped) {
...
future<> schema_applier::notify() {
    ...
    co_await sharded_db.invoke_on_all([&] (replica::database& db) -> future<> {
        ...
        for (auto& name : _affected_keyspaces.names.created) {
            co_await notifier.create_keyspace(name);
        }
        for (auto& name : _affected_keyspaces.names.altered) {
            co_await notifier.update_keyspace(name);
        }
        for (auto& name : _affected_keyspaces.names.dropped) {
