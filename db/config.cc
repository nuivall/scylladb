...
db::config::config(std::shared_ptr<db::extensions> exts)
    ...
    */
    ...
    , max_uninitialized_connections_per_shard(this, "max_uninitialized_connections_per_shard", liveness::LiveUpdate, value_status::Used, 100,
        "Maximum number of new connections from drivers that a single shard can handle before it starts shedding excess connections. This limit applies only to new connections; connections that are ready to serve requests are not affected. By default the limit is 100 new connections per second.")
