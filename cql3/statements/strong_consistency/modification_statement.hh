...
#include "cql3/statements/insert_statement.hh"
...
// Builds the single mutation a strongly consistent modification statement
// produces for the given partition key and timestamp. Shared by single
// statements and batches, which build one mutation per statement and merge them.
mutation build_mutation(const cql3::statements::modification_statement& stmt, const query_options& options,
        api::timestamp_type ts, const cql3::statements::modification_statement::json_cache_opt& json_cache,
        const std::vector<dht::partition_range>& keys);

// Builds either Stmt or its strongly consistent counterpart, depending on the
// keyspace the statement targets. Typed as Stmt, because the caller goes on to
// use the statement kind's own interface to fill it in.
