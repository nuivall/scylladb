...
void batch_statement::validate_strongly_consistent() const {
    ...
    // The Raft group assigns the timestamp, so a batch-level one cannot be
    // honoured; per-statement timestamps are already rejected when the
        ...
        bool, api::timestamp_type) const
{
    ...
    auto mutate_result = co_await coordinator.get().mutate(batch_schema,
        ...
        [&](api::timestamp_type ts) {
            ...
            for (size_t i = 0; i < statements.size(); ++i) {
                const auto& stmt = *statements[i].statement;
                auto m = build_mutation(stmt, options.for_statement(i), ts, all_keys[i].json_cache, all_keys[i].keys);
                if (!merged) {
                    merged = std::move(m);
                } else {
