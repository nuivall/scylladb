...
query_processor::execute_batch_without_checking_exception_message(
    ...
    const auto& statements = stmt->get_statements();
    _stats.queries_by_cl[size_t(options.get_consistency())] += statements.size();

    if (log.is_enabled(logging::log_level::trace)) {
        std::ostringstream oss;
        for (const auto& s : statements) {
            if (const auto* inner = s.statement->get_audit_info()) {
                oss << std::endl << inner->query();
            }
        }
        log.trace("execute_batch({}): {}", statements.size(), oss.str());
    }
