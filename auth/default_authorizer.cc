...
future<> default_authorizer::revoke_all(const resource& resource) const {
    ...
    try {
        ...
        auto gen = [this, name, timestamp] () -> mutations_generator {
            static const sstring query = format("SELECT {} FROM {}.{} WHERE {} = ? ALLOW FILTERING",
                ROLE_NAME,
                _auth_ks_name,
                PERMISSIONS_CF,
        ...
        co_await announce_mutations_with_batching(_qp,
            _group0_client,
            db::system_auth_keyspace::role_permissions(),
            std::move(group0_guard),
            std::move(gen),
            &_as);
    } catch (exceptions::request_execution_exception& e) {
