...
public:
    explicit auth_migration_listener(authorizer& a, cql3::query_processor& qp)
        : _authorizer(a)
        , _qp(qp) {
    }
...
private:
    ...
    void on_drop_column_family(const sstring& ks_name, const sstring& cf_name) override {
        ...
        (void)do_with(::service::group0_batch::unused(), [this, &ks_name, &cf_name](auto& mc) mutable {
            return _authorizer.revoke_all(auth::make_data_resource(ks_name, cf_name), mc);
        }).handle_exception([](std::exception_ptr e) {
    ...
    void on_drop_function(const sstring& ks_name, const sstring& function_name) override {
        ...
        (void)do_with(::service::group0_batch::unused(), [this, &ks_name, &function_name](auto& mc) mutable {
            return _authorizer.revoke_all(auth::make_functions_resource(ks_name, function_name), mc);
        }).handle_exception([](std::exception_ptr e) {
    ...
    void on_drop_aggregate(const sstring& ks_name, const sstring& aggregate_name) override {
        ...
        (void)do_with(::service::group0_batch::unused(), [this, &ks_name, &aggregate_name](auto& mc) mutable {
            return _authorizer.revoke_all(auth::make_functions_resource(ks_name, aggregate_name), mc);
        }).handle_exception([](std::exception_ptr e) {
...
service::service(utils::loading_cache_config c, cache& cache, cql3::query_processor& qp, ::service::raft_group0_client& g0, ::service::migration_notifier& mn,
        std::unique_ptr<authorizer> z, std::unique_ptr<authenticator> a, std::unique_ptr<role_manager> r, maintenance_socket_enabled used_by_maintenance_socket)
    ...
    , _permissions_cache_cfg_cb([this](uint32_t) {
        (void)_permissions_cache_config_action.trigger_later();
    })
    , _permissions_cache_config_action([this] {
        update_cache_config();
...
service::service(utils::loading_cache_config c, cql3::query_processor& qp, ::service::raft_group0_client& g0, ::service::migration_notifier& mn,
        ::service::migration_manager& mm, const service_config& sc, maintenance_socket_enabled used_by_maintenance_socket, cache& cache)
    : service(std::move(c), cache, qp, g0, mn, create_object<authorizer>(sc.authorizer_java_name, qp, g0, mm),
              create_object<authenticator>(sc.authenticator_java_name, qp, g0, mm, cache),
              create_object<role_manager>(sc.role_manager_java_name, qp, g0, mm, cache), used_by_maintenance_socket) {
}
    ...
    while (!db.has_keyspace(meta::legacy::AUTH_KS)) {
        ...
        if (!db.has_keyspace(meta::legacy::AUTH_KS)) {
            ...
            auto ksm = data_dictionary::keyspace_metadata::new_keyspace(
                    meta::legacy::AUTH_KS, "org.apache.cassandra.locator.SimpleStrategy", opts, std::nullopt, std::nullopt);

            try {
                co_return co_await mm.announce(::service::prepare_new_keyspace_announcement(db.real_database(), ksm, ts), std::move(group0_guard),
                        seastar::format("auth_service: create {} keyspace", meta::legacy::AUTH_KS));
...
future<> service::start(::service::migration_manager& mm, db::system_keyspace& sys_ks) {
    ...
    _access_checker = std::make_unique<cql3::access_checker>(_qp, *this);
    _qp.set_authorized_cache_access_checker(_access_checker.get());
...
future<> service::stop() {
    ...
    // Stop the access checker first: close its gate (waits for in-flight
    // check_access calls to complete), then unplug from the authorized
    // prepared statements cache. This ensures no more cache reload callbacks
    // will use auth::service after this point.
    if (_access_checker) {
        co_await _access_checker->stop();
        _qp.set_authorized_cache_access_checker(nullptr);
    }
...
future<permission_set> service::get_uncached_permissions(const role_or_anonymous& maybe_role, const resource& r) const {
...
static void validate_authentication_options_are_supported(const authentication_options& options, const authentication_option_set& supported) {
    ...
    if (options.credentials) {
        std::visit(make_visitor(
                           [&](const password_option&) {
                               check(authentication_option::password);
                           },
                           [&](const hashed_password_option&) {
...
future<> service::create_role(std::string_view name, const role_config& config, const authentication_options& options, ::service::group0_batch& mc) const {
    ...
    try {
        validate_authentication_options_are_supported(options, underlying_authenticator().supported_options());
...
future<std::vector<cql3::description>> service::describe_roles(bool with_hashed_passwords) {
    ...
    auto produce_create_statement = [with_hashed_passwords](const sstring& formatted_role_name, const std::optional<sstring>& maybe_hashed_password,
                                            bool can_login, bool is_superuser) {
        ...
        const sstring role_part =
                formatted_role_name == meta::DEFAULT_SUPERUSER_NAME ? seastar::format("IF NOT EXISTS {}", formatted_role_name) : formatted_role_name;
        ...
        return seastar::format("CREATE ROLE {} {} LOGIN = {} AND SUPERUSER = {};", role_part, with_hashed_password_part, can_login, is_superuser);
    };
    ...
    for (const auto& role : roles) {
        ...
        result.push_back(cql3::description{// Roles do not belong to any keyspace.
...
static sstring describe_udf_resource(const permission& perm, const resource& r, std::string_view role) {
    ...
    if (maybe_fun_name) {
        ...
        return seastar::format("GRANT {} ON FUNCTION {}.{}({}) TO {};", permission, ks, fun_name, fmt::join(fun_args_range, ", "), formatted_role);
    }
    ...
    return seastar::format("GRANT {} ON FUNCTION {}.{}({}) TO {};", permission, ks, fun_name, fmt::join(parsed_fun_args, ", "), formatted_role);
}
...
future<std::vector<cql3::description>> service::describe_permissions() const {
    ...
    for (const auto& permissions : permission_list) {
        for (const auto& permission : permissions.permissions) {
            ...
            result.push_back(cql3::description{// Permission grants do not belong to any keyspace.
                    .keyspace = std::nullopt,
...
future<> create_role(const service& ser, std::string_view name, const role_config& config, const authentication_options& options, ::service::group0_batch& mc) {
    return ser.create_role(name, config, options, mc);
}

future<> alter_role(const service& ser, std::string_view name, const role_config_update& config_update, const authentication_options& options,
    ...
    validate_authentication_options_are_supported(options, ser.underlying_authenticator().supported_options());
...
future<bool> has_role(const service& ser, std::string_view grantee, std::string_view name) {
    return when_all_succeed(validate_role_exists(ser, name), ser.get_roles(grantee)).then_unpack([name](role_set all_roles) {
...
future<> set_attribute(
        const service& ser, std::string_view role_name, std::string_view attribute_name, std::string_view attribute_value, ::service::group0_batch& mc) {
...
future<> grant_permissions(const service& ser, std::string_view role_name, permission_set perms, const resource& r, ::service::group0_batch& mc) {
...
future<> revoke_permissions(const service& ser, std::string_view role_name, permission_set perms, const resource& r, ::service::group0_batch& mc) {
    co_await validate_role_exists(ser, role_name);
...
future<std::vector<permission_details>> list_filtered_permissions(const service& ser, permission_set perms, std::optional<std::string_view> role_name,
        const std::optional<std::pair<resource, recursive_permissions>>& resource_filter) {
    return ser.underlying_authorizer().list_all().then([&ser, perms, role_name, &resource_filter](std::vector<permission_details> all_details) {
        if (resource_filter) {
            const resource r = resource_filter->first;

            const auto resources = resource_filter->second ? auth::expand_resource_family(r) : auth::resource_set{r};
        ...
        std::transform(
                std::make_move_iterator(all_details.begin()), std::make_move_iterator(all_details.end()), all_details.begin(), [perms](permission_details pd) {
...
future<> migrate_to_auth_v2(db::system_keyspace& sys_ks, ::service::raft_group0_client& g0, start_operation_func_t start_operation_func, abort_source& as) {
    ...
    auto gen = [&sys_ks](api::timestamp_type ts) -> ::service::mutations_generator {
        ...
        for (const auto& cf_name : std::vector<sstring>{"roles", "role_members", "role_attributes", "role_permissions"}) {
            schema_ptr schema;
            try {
            ...
            co_await qp.query_internal(
                    seastar::format("SELECT * FROM {}.{}", meta::legacy::AUTH_KS, cf_name), db::consistency_level::ALL, {}, 1000,
                    [&qp, &cf_name, &col_names, &val_binders_str, &schema, ts, &collected](const cql3::untyped_result_set::row& row) -> future<stop_iteration> {
                        ...
                        auto muts = co_await qp.get_mutations_internal(seastar::format("INSERT INTO {}.{} ({}) VALUES ({})", db::system_keyspace::NAME, cf_name,
                                                                               fmt::join(col_names, ", "), val_binders_str),
                                internal_distributed_query_state(), ts, std::move(values));
                        if (muts.size() != 1) {
        ...
        co_yield co_await sys_ks.make_auth_version_mutation(ts, db::system_keyspace::auth_version_t::v2);
    };
    co_await announce_mutations_with_batching(g0, start_operation_func, std::move(gen), as, std::nullopt);
}
