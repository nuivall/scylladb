...
write_locked_global_table_ptr::write_locked_global_table_ptr(global_table_ptr&& ptr) : global_table_ptr(std::move(ptr)) {}

void write_locked_global_table_ptr::assign_lock(seastar::rwlock::holder&& h) {
...
future<write_locked_global_table_ptr> database::prepare_drop_table_on_all_shards(sharded<database>& sharded_db, sstring ks_name, sstring cf_name) {
    auto uuid = sharded_db.local().find_uuid(ks_name, cf_name);
    write_locked_global_table_ptr table_shards = co_await get_table_on_all_shards(sharded_db, uuid);
    co_await sharded_db.invoke_on_all([&] (auto& db) -> future<> {
        table_shards.assign_lock(co_await db.get_tables_metadata().hold_write_lock());
    });
    co_return table_shards;
}

void database::drop_table(sharded<database>& sharded_db,
        sstring ks_name, sstring cf_name, bool with_snapshot, write_locked_global_table_ptr& table_shards) {
...
future<> database::cleanup_drop_table_on_all_shards(sharded<database>& sharded_db, sharded<db::system_keyspace>& sys_ks,
        bool with_snapshot, write_locked_global_table_ptr& table_shards) {
...
future<std::unique_ptr<keyspace>> database::create_in_memory_keyspace(const lw_shared_ptr<keyspace_metadata>& ksm, locator::effective_replication_map_factory& erm_factory, system_keyspace system) {
    ...
    std::unique_ptr<keyspace> ks(std::make_unique<keyspace>(std::move(kscfg), erm_factory));
