...
def test_reject_conditions(cql, sc_keyspace):
    """
    Strongly consistent writes are committed unconditionally through Raft, so
    IF clauses cannot be honoured and must be rejected rather than ignored.
    """
    with new_test_table(cql, sc_keyspace, "pk int PRIMARY KEY, v int") as table:
        ...
        with pytest.raises(InvalidRequest, match=error_msg):
            cql.execute(f"UPDATE {table} SET v = 13 WHERE pk = 0 IF v = 12")
        with pytest.raises(InvalidRequest, match=error_msg):
            cql.execute(f"DELETE FROM {table} WHERE pk = 0 IF EXISTS")
        with pytest.raises(InvalidRequest, match=error_msg):
            cql.prepare(f"INSERT INTO {table} (pk, v) VALUES (?, ?) IF NOT EXISTS")
        with pytest.raises(InvalidRequest, match=error_msg):
