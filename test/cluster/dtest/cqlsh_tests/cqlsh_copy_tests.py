...
class TestCqlshCopy(CqlshPrepare):
    ...
    def _test_bulk_round_trip(  # noqa: PLR0913, PLR0915
        ...
        # When copy_from_retry is provided, we're testing with timeouts and retries.
        # The test may iterate multiple times with different parameters to find a configuration
        # where retries occur AND the round-trip succeeds, so we skip this strict first-export check.
        if not copy_from_retry:
    ...
    @pytest.mark.single_node
    @pytest.mark.use_cassandra_stress
    def test_bulk_round_trip_with_timeouts(self):  # noqa: PLR0912
        """
        ...
        if (type(self.cluster) is ScyllaCluster and self.cluster.scylla_mode == "debug") or is_coverage(self.cluster.get_install_dir()):
            num_operations //= 10
        ...
        self._test_bulk_round_trip(
            ...
            copy_from_options={"MAXINSERTERRORS": -1, "MAXATTEMPTS": max_attempts, "NUMPROCESSES": numprocesses, "CHUNKSIZE": chunksize, "REQUESTTIMEOUT": 600},
        ...
        while numprocesses < 16:
            ...
            second_try_count = 0
            while (copy_from_retry[-1] != True and chunksize < max_chuncksize) or (second_try_count < 20 and copy_from_success[-1] != True):
                self._test_bulk_round_trip(
                    nodes=1,
                    partitioner="murmur3",
                    num_operations=100000,
