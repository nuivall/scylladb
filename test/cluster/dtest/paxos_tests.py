...
class TestPaxos(Tester):
    ...
    # Upstream deselects it with @pytest.mark.require("jira:DTEST-21").  The
    # "removed" nodes are only killed, so they stay in the topology as dead,
    # and the topology coordinator rejects every later join ("request canceled
    # because some required nodes are dead").
    @pytest.mark.skip_bug(link="https://scylladb.atlassian.net/browse/DTEST-21",
                          reason="bootstrapping new nodes while other nodes are down is rejected")
