...
struct frame_builder {
    ...
    void write_int(int32_t v) {
        char buf[4];
        write_be<int32_t>(buf, v);
        body.write(buf, 4);
    }
    void write_short(uint16_t v) {
        char buf[2];
        write_be<uint16_t>(buf, v);
        body.write(buf, 2);
    }
...
public:
    ...
    future<frame> read_one_frame_internal() {
        ...
        sstring payload(sstring::initialized_later(), len);
        std::memcpy(payload.begin(), body.get(), len);
        co_return frame{opcode, stream, std::move(payload)};
    }
    ...
    future<> startup() {
        ...
        if (op != cql_binary_opcode::READY && op != cql_binary_opcode::AUTH_SUCCESS) {
            ...
            if (op == cql_binary_opcode::ERROR && payload.size() >= 4) {
                int32_t code = read_be<int32_t>(payload.data());
                // message string follows: <string>
                if (payload.size() >= 6) {
                    auto p = payload.data() + 4;
    ...
    future<> query_simple(std::string_view q) {
        ...
        if (f.opcode == cql_binary_opcode::ERROR) {
            throw std::runtime_error(format("server returned ERROR to QUERY: {}", f.payload));
        }
    }
    ...
    future<sstring> prepare_query(std::string_view q) {
        ...
        int32_t kind = read_be<int32_t>(payload.data());
        ...
        uint16_t id_len = read_be<uint16_t>(payload.data() + 4);
        if (payload.size() < 6 + id_len) throw std::runtime_error("short PREPARED id");
        sstring id(payload.data() + 6, id_len);
        co_return id;
    }
    ...
    future<> execute_prepared(const sstring& id, std::string_view key) {
        ...
        fb.body.write(key.data(), key.size());
...
std::function<int(int, char**)> perf_cql_raw(std::function<int(int, char**)> scylla_main, std::function<void(lw_shared_ptr<db::config>)>* after_init_func) {
    return [=](int ac, char** av) -> int {
        ...
        *after_init_func = [c](lw_shared_ptr<db::config> cfg) mutable {
            ...
            static std::optional<future<>> run_future;
            run_future = seastar::async([c]() {
                try {
                    workload_main(c);
                } catch (...) {
