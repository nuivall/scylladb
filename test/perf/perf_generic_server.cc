 ...
 */
 ...
 public:
     test_connection(generic_server::server& server, connected_socket&& fd, lw_shared_ptr<named_semaphore> sem, semaphore_units<named_semaphore_exception_factory> initial_sem_units, const test_config& conf)
 ...
 public:
     virtual shared_ptr<generic_server::connection> make_connection(socket_address server_addr, connected_socket&& fd, socket_address addr, lw_shared_ptr<named_semaphore> sem, semaphore_units<named_semaphore_exception_factory> initial_sem_units) override {
         return make_shared<test_connection>(*this, std::move(fd), std::move(sem), std::move(initial_sem_units), _conf);
     }

     test_server(const test_config& conf) : generic_server::server("test_server", plog, *conf.db_config), _conf(conf) {}
 };
 ...
 int main(int argc, char** argv) {
     ...
     app_template app;
     app.add_options()
         ("duration", bpo::value<unsigned>()->default_value(3), "seconds to run")
         ("concurrency", bpo::value<unsigned>()->default_value(200), "clients per shard")
         ("requests-per-connection", bpo::value<uint64_t>()->default_value(1), "number of requests issued before closing the connection and making new one")
